...
async def settings_view(session: AsyncSession, user: User) -> tuple[str, InlineKeyboardMarkup]:
    ...
    kb = markup([
        ...
        [btn(t("set.manage_sub") if access.kind == "paid" else t("btn.pay"), Bl(a="open"))],
...
async def send_settings(message: Message, session: AsyncSession, user: User, settings: Settings) -> None:
    text, kb = await settings_view(session, user)
...
async def st_back(cb: CallbackQuery, session: AsyncSession, state: FSMContext, user: User) -> None:
    await state.clear()
    await cb.answer()
    await _edit(cb, *await settings_view(session, user))


@router.callback_query(St.filter(F.a == "lang"))
async def st_lang(cb: CallbackQuery, bot: Bot, session: AsyncSession, user: User) -> None:
...
async def st_tz_set(cb: CallbackQuery, callback_data: St, session: AsyncSession, user: User) -> None:
...
async def in_tz(message: Message, session: AsyncSession, state: FSMContext, user: User) -> None:
