...
async def plan_view(session: AsyncSession, user: User, day: date | None = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    rows = [[
        btn("◀️", Cp(a="day", d=ordinal - 1)) if day > today else btn("·", Cp(a="noop")),
        btn(fmt_date(day, user.lang), Cp(a="noop")),
        btn("▶️", Cp(a="day", d=ordinal + 1)),
    ]]
    ...
    for pub in pubs:
        ...
        if post is None or not post.parts:
            continue
        first = post.parts[0]
        label = f"{fmt_hm(pub.run_at.astimezone(zone))} {part_icon(first)} {snippet(first.text_html, 30) or t('parts.no_text')}"
        if pub.status == "paused":
            label = "⏸ " + label
        rows.append([btn(label, Cp(a="post", d=ordinal, id=post.id))])
    lines.append(t("plan.count", n=len(seen)) if seen else t("plan.empty"))
...
async def cp_post(cb: CallbackQuery, callback_data: Cp, session: AsyncSession, user: User) -> None:
    ...
    for pub in await pubs_repo.pending_between(session, user.id, start, end):
        if pub.post_id != post.id:
            continue
        channel = await session.get(Channel, pub.channel_id)
        local = pub.run_at.astimezone(zone)
        lines.append(f"📡 {html.escape(channel.title if channel else '?')} — {fmt_date(local.date(), user.lang)} {fmt_hm(local)}")
    if post.repeat and post.repeat.active:
        lines.append(t("ed.sum_repeat", interval=fmt_interval(post.repeat.interval_minutes)))
    p, d = post.id, callback_data.d
    kb = markup([
        [btn(t("plan.edit"), Cp(a="edit", d=d, id=p)), btn(t("plan.move"), Cp(a="move", d=d, id=p))],
        [btn(t("plan.now"), Cp(a="now", d=d, id=p))],
        [btn(t("plan.drop"), Cp(a="drop", d=d, id=p))],
        [btn(t("btn.back"), Cp(a="day", d=d))],
    ])
    await _edit(cb, "\n".join(lines), kb)
