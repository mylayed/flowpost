...
const I18N = {
  uk: {
    ...
    wm_line: "Водяні знаки: {photo} фото · {video} відео · AI-текстів: {ai}",
  ...
  en: {
    ...
    wm_line: "Watermarks: {photo} photo · {video} video · AI texts: {ai}",
  ...
  const planRows = (ui.expanded ? p.posting : p.posting.slice(0, 2)).map((plan) =>
    h("div", { class: "plan-row" },
      h("div", {},
        ...
        h("div", { class: "plan-sub" },
          t("wm_line", { photo: number(plan.wm_photo), video: number(plan.wm_video), ai: number(plan.ai_text) }))),
  ...
  api(`channels/${id}`)
    .then((c) => {
      ...
      card.replaceChildren(
        ...
        h("div", { class: "remaining channel-quotas" },
