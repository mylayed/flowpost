...
async def broadcast_recipients(
    ...
    """(user id, Telegram id, language) of everyone a /broadcast goes to: "all" — every user who hasn't blocked the bot,
    ...
    stmt = select(User.id, User.tg_id, User.lang).where(User.is_blocked.is_(False), User.id > after_user_id)
    ...
    if audience == "owners":
        stmt = stmt.where(is_owner)
    elif audience == "channels":
        stmt = stmt.where(has_channels)
    elif audience == "nochannels":
        stmt = stmt.where(~has_channels)
